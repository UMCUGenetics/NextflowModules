process FilterConsensusReads {
    ...
    input:
        tuple (val(sample_id), flowcell, machine, run_nr, path(bam))

    output:
        tuple (val(sample_id), flowcell, machine, run_nr, path("${sample_id}.u.consensus.filtered.bam"), emit: filtered_bams)
